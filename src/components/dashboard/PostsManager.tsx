    ...
    if (block.type === "text") {
      return (
        <div key={block.id} className="space-y-2">
          ...
          <p className="whitespace-pre-wrap break-words text-[] leading-8 text-slate-300 sm:text-base">
            {block.value}
          </p>
        </div>
      );
    ...
    if (block.type === "image") {
      return (
        ...
        >
          ...
          <div className="flex min-h-[] items-center justify-center bg-black/20 p-2 sm:min-h-[] sm:p-4">
            <img
              src={block.value}
              alt={block.title || "Post image"}
              className="max-h-[] w-full rounded-xl object-contain"
              loading="lazy"
            />
          </div>
        </div>
      ...
      return (
        ...
        >
          ...
          <iframe
            ...
            className="h-[] w-full bg-white sm:h-[] lg:h-[]"
          />
        </div>
      );
    }
...
export default function PostsManager() {
  ...
  const [postStatusFilter, setPostStatusFilter] = useState<"all" | "published" | "draft" | "featured">("all");
  ...
  const visiblePosts = useMemo(() => {
    ...
    return posts.filter((post) => {
      const matchesQuery = !query || `${post.title} ${post.content} ${post.tags.join(" ")}`.toLowerCase().includes(query);
      const matchesStatus = postStatusFilter === "all" || (postStatusFilter === "featured" ? post.featured : post.status === postStatusFilter);
  ...
  function startEditPost(post: Post) {
    ...
    setBlocks(parsedBlocks.length ? parsedBlocks : [createBlock("text")]);
  ...
  function copyPostLink(postId: string) {
    const url = `${window.location.origin}/dashboard/posts#post-${postId}`;
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#020617] px-4 py-20 text-white sm:px-6 lg:px-8">
      ...
      <div
        aria-hidden="true"
        className="pointer-events-none absolute left-1/2 top-0 h-[] w-[] -translate-x-1/2 rounded-full bg-blue-600/[0.12] blur-[]"
      />

      <div
        aria-hidden="true"
        className="pointer-events-none absolute bottom-0 right-[-] h-[] w-[] rounded-full bg-cyan-500/[0.08] blur-[]"
      />
      ...
      <div
        ...
        style={{
          backgroundImage:
            "linear-gradient(rgba(255,255,255,.5) , transparent ), linear-gradient(90deg, rgba(255,255,255,.5) , transparent )",
          backgroundSize: " ",
        }}
      />
      ...
      <div className="relative z-10 mx-auto w-full max-w-7xl">
        ...
        <section className="mb-7 grid gap-3 md:grid-cols-[1fr_190px]">
          <input value={postSearch} onChange={(event)=>setPostSearch(event.target.value)} placeholder="Search posts by title, content, or tag..." className="rounded-xl border border-white/10 bg-white/[0.035] px-4 py-3 text-sm text-white outline-none placeholder:text-slate-600 focus:border-cyan-400/40" />
          <select value={postStatusFilter} onChange={(event)=>setPostStatusFilter(event.target.value as "all" | "published" | "draft" | "featured")} className="rounded-xl border border-white/10 bg-slate-950 px-4 py-3 text-sm text-white"><option value="all">All posts</option><option value="published">Published</option><option value="draft">Draft</option><option value="featured">Featured</option></select>
        </section>
        ...
        {posts.length > 0 && visiblePosts.length === 0 && (
          <div className="mb-7 rounded-2xl border border-dashed border-white/10 p-8 text-center text-slate-500">No posts match the current search/filter.</div>
        )}
          ...
          {visiblePosts.map((post) => {
            ...
            return (
              <article
                id={`post-${post.id}`}
                key={post.id}
                className="overflow-hidden rounded-[] border border-white/10 bg-slate-950/65 shadow-2xl shadow-black/30 backdrop-blur-2xl"
              >
                {/* Post Header */}
                <div className="p-5 sm:p-7">
                      ...
                      {authenticated && (
                        <>
                          <button
                            ...
                            className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2 text-xs font-semibold text-slate-400 transition hover:border-cyan-400/20 hover:bg-cyan-400/[0.06] hover:text-cyan-300"
                  ...
                  {authenticated && (post.status === "draft" || post.featured || post.tags.length > 0) && (
                    <div className="mt-3 flex flex-wrap gap-2">
                      {post.status === "draft" && (
                        <span className="rounded-full border border-amber-400/20 bg-amber-400/10 px-2.5 py-1 text-[] font-bold uppercase tracking-wider text-amber-200">Draft</span>
                      )}
                      {post.featured && (
                        <span className="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-2.5 py-1 text-[] font-bold uppercase tracking-wider text-cyan-200">Featured</span>
                      )}
                      {post.tags.map((tag) => (
                        <span key={tag} className="rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-1 text-[] text-slate-400">#{tag}</span>
                      ))}
                    </div>
                  )}
                  ...
                  {post.imageUrl && (
                    <div className="mt-6 overflow-hidden rounded-2xl border border-cyan-400/10 bg-black/30">
                      <img
                        src={post.imageUrl}
                        alt={post.title}
                        className="max-h-[] w-full object-contain"
                        loading="lazy"
                      />
                    </div>
                  )}
      ...
      {authenticated && showCreatePost && (
        <div className="fixed inset-0 z-[100] overflow-y-auto bg-black/80 p-3 backdrop-blur-md sm:p-5">
          <div className="flex min-h-full items-center justify-center py-4">
            <div className="w-full max-w-5xl overflow-hidden rounded-[] border border-cyan-400/15 bg-[#04101f] shadow-2xl shadow-cyan-500/10">
              {/* Modal header */}
              <div className="sticky top-0 z-20 flex items-center justify-between border-b border-white/10 bg-[#04101f]/95 p-5 backdrop-blur-xl sm:p-6">
                <div>
                  <div className="mb-2 inline-flex rounded-full border border-cyan-400/20 bg-cyan-400/[0.06] px-3 py-1 text-[] font-black uppercase tracking-[0.18em] text-cyan-300">
                    Content Studio
                  </div>
                  ...
                  <p className="mt-1 text-xs text-slate-600">
                    {editingPostId
                      ? "Update the existing post without changing its current expiry time."
                      : "Paste any value and the editor will automatically detect its type."}
                  </p>
                </div>
...
https://example.com`}
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div>
                        ...
                        <select
                          value={postStatus}
                          onChange={(event) => setPostStatus(event.target.value as "draft" | "published")}
                      ...
                      <label className="flex items-end">
                        <span className="flex w-full items-center gap-3 rounded-xl border border-white/10 bg-white/[0.035] px-4 py-3 text-sm font-semibold text-slate-300">
                          <input
                            type="checkbox"
                            checked={featured}
                    ...
                    <div>
                      <label className="text-sm font-bold text-slate-300">Tags <span className="font-normal text-slate-600">comma separated</span></label>
                      <input
                        type="text"
                        value={tagsText}
                        onChange={(event) => setTagsText(event.target.value)}
                        placeholder="nextjs, portfolio, tutorial"
                        className="mt-2 w-full rounded-xl border border-white/10 bg-white/[0.035] px-4 py-3 text-white outline-none placeholder:text-slate-700 focus:border-cyan-400/40"
                      />
                    </div>
                  ...
                  <div className="min-w-0">
                    <div className="mb-4 flex items-center justify-between gap-3">
                      ...
                      <div className="rounded-full border border-cyan-400/15 bg-cyan-400/[0.05] px-3 py-1.5 text-[] font-black uppercase tracking-wider text-cyan-300">
                        Auto Layout
                      </div>
                    </div>
                            ...
                            <div className="space-y-3 p-4">
                              ...
                              {block.value.trim() && (
                                <div className="rounded-xl border border-cyan-400/10 bg-black/20 p-3">
                                  <p className="mb-2 text-[] font-black uppercase tracking-[0.15em] text-slate-700">
                                    Preview
                                  </p>
