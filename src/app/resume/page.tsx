...
export default async function ResumePage() {
  const data = await getLocalData<ResumeData>("Resume.json");
  if (!data) {
    return (
      <main className="relative min-h-screen overflow-hidden text-white">
        <div className="flex min-h-[400px] flex-col items-center justify-center p-8 text-center">
          ...
          <p className="mt-2 text-sm text-gray-400">Failed to load resume data.</p>
          ...
          <div className="grid gap-10 px-6 py-10 sm:px-10 lg:grid-cols-[1fr_300px] lg:px-14">
            ...
            <div className="space-y-10">
              ...
              {data.experiences && data.experiences.length > 0 && (
                <ResumeSection icon={<FaBriefcase />} title="Professional Experience">
                  <div className="space-y-5">
                    {data.experiences.map((exp) => (
                      <article key={`${exp.role}-${exp.company}`} className="group rounded-2xl border border-white/10 bg-white/[0.035] p-5 transition-all duration-300 hover:border-cyan-400/25 hover:bg-white/[0.055]">
                        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                          <div>
                            <h3 className="text-lg font-bold text-white transition group-hover:text-cyan-200">{exp.role}</h3>
                            <p className="mt-1 font-semibold text-cyan-300">{exp.company}</p>
                            <p className="mt-1 text-sm text-gray-500">{exp.location}</p>
                          </div>
                          <span className="w-fit rounded-full border border-blue-400/20 bg-blue-400/10 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-blue-300">{exp.period}</span>
                        </div>
                        <ul className="mt-4 space-y-2 text-sm text-gray-400">
                          {exp.description.map((item, i) => (
                            <li key={i} className="flex items-start gap-2"><span className="mt-1 h-1.5 w-1.5 rounded-full bg-cyan-400" />{item}</li>
                          ))}
                        </ul>
                      </article>
                    ))}
                ...
                <ResumeSection icon={<FaCode />} title="Featured Projects">
                  <div className="space-y-5">
                    {data.projects.map((project) => (
                      <article key={project.name} className="group rounded-2xl border border-white/10 bg-white/[0.035] p-5 transition-all duration-300 hover:-translate-y-1 hover:border-cyan-400/25 hover:bg-white/[0.055]">
                ...
                <ResumeSection icon={<FaGraduationCap />} title="Education">
                  <div className="space-y-4">
                    {data.education.map((edu) => (
                      <EducationRow key={edu.title} title={edu.title} institution={edu.institution} location={edu.location} status={edu.status} details={edu.details} />
                    ))}
                  </div>
                </ResumeSection>
              )}
            ...
            <aside className="space-y-8">
              ...
              {data.careerFocus && (
                <SideSection title="Career Focus">
                  <p className="text-sm leading-7 text-gray-400">{data.careerFocus}</p>
                </SideSection>
              )}
              {/* Availability */}
              {data.availability && (
                <SideSection title="Availability">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-semibold text-emerald-300">{data.availability.status}</span>
                  </div>
                  <p className="mt-3 text-sm leading-6 text-gray-500">{data.availability.description}</p>
                </SideSection>
              )}
              {/* Contact */}
              {data.contact && (
                <SideSection title="Contact">
                  <div className="space-y-4">
                    <a href={`tel:${data.contact.phone.replace(/[^+\\d]/g, "")}`} className="flex items-start gap-3 text-sm text-gray-400 transition hover:text-cyan-300"><FaPhoneAlt className="mt-1 shrink-0 text-emerald-400" />{data.contact.phone}</a>
                    <a href={`mailto:${data.contact.email}`} className="flex items-start gap-3 break-all text-sm text-gray-400 transition hover:text-cyan-300"><FaEnvelope className="mt-1 shrink-0 text-blue-400" />{data.contact.email}</a>
                    <p className="flex items-start gap-3 text-sm text-gray-400"><FaMapMarkerAlt className="mt-1 shrink-0 text-pink-400" />{data.contact.address}</p>
                  </div>
                </SideSection>
              )}
            </aside>
...
function EducationRow({ title, institution, location, status, details }: EducationRowProps) {
  ...
  return (
    <article className="rounded-2xl border border-white/10 bg-white/[0.035] p-5">
      ...
      {details && (
        <p className="mt-3 text-sm leading-6 text-gray-400">{details}</p>
      )}
    </article>
  );
}
